...
from llava.train.llava_trainer import LLaVATrainer, LLavaDPOTrainer

from llava import conversation as conversation_lib
from llava.model import *
from llava.mm_utils import tokenizer_image_token
# from llava.eval.muffin_inference_logp import preference_collator_fn
...
# The following function is not used!!1 
def preprocess_llama_2(
    ...
    # Mask targets
    sep = "[/INST] "
    skip_computing_loss_on_completion_turn = defaultdict(lambda: False)
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if skip_computing_loss_on_completion_turn[i]:
                # Ignore the entire round length, including the instruction and the completion
                target[cur_len : cur_len + round_len] = IGNORE_INDEX
            else:
                # Compute loss only on the completion part, the typical case
...
def preprocess_v1(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        rounds = conversation.split(conv.sep2)
        # assert len(rounds) == 4, f"{[f'-------------------{i}: {rou}' for i, rou in enumerate(rounds)]}"
        cur_len = 1
        target[:cur_len] = IGNORE_INDEX
        for i, rou in enumerate(rounds):
            if rou == "":
            ...
            if skip_computing_loss_on_completion_turn[i]:
                # Ignore the entire round length, including the instruction and the completion
                print(f"skip_computing_loss_on_completion_turn[{i}] = True")
                target[cur_len : cur_len + round_len] = IGNORE_INDEX
            else:
                # Compute loss only on the completion part, the typical case
                target[cur_len : cur_len + instruction_len] = IGNORE_INDEX
...
class DataCollatorForSupervisedDataset(object):
    ...
    Similar to huggingface tokenizer module output, takes in a sequence of input_ids and labels wrapped as a sequence of dict."""
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        print(f'Inside Supervised Dataset: {instances[0].keys()}')
...
class DPODataset(Dataset):
    def __init__(self,
        ...
        self.tokenizer = tokenizer
        # Assume data path already exist
        self.list_data_dict = json.load(open(multimodal_cfg['data_path'], "r"))
...
def expand_image_token(source, multimodal_cfg) -> Dict:
    ...
    # image_token_len = multimodal_cfg['image_token_len'] # TODO missing
    if not is_multimodal:
        return source
    DEFAULT_IMAGE_PATCH_TOKEN = "<im_patch>"
    # Will replace the <image> with duplicated number, but not applicable to llava case
    # for sentence in source:
...
def expand2square(pil_img, background_color):
    if isinstance(pil_img, float):
        print(f'Image is a float: {pil_img}')
    width, height = pil_img.size
    if width == height:
        return pil_img
    elif width > height:
...
def encode_multimodal_preference_sample(source, tokenizer, multimodal_cfg):
    if isinstance(source['chosen'], list):
        ...
        if isinstance(source['question'], str):
    ...
    if 'image' in source:
        ...
        if isinstance(source['image'], str):
        ...
        ### StART OF MY EXPANIOSN 
        if multimodal_cfg['image_aspect_ratio'] == 'pad':
            image = expand2square(image, tuple(int(x*255) for x in processor.image_mean))
            image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
        else:
            image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
        # End of my expanion to take care of the expansion logic
        win_conv = expand_image_token(win_conv, multimodal_cfg)
        rej_conv = expand_image_token(rej_conv, multimodal_cfg)

    rej_data_dict = preprocess([rej_conv], tokenizer, has_image=('image' in source))
    rej_data_dict = dict(input_ids=rej_data_dict["input_ids"][0],
                         labels=rej_data_dict["labels"][0])

    win_data_dict = preprocess([win_conv], tokenizer, has_image=('image' in source))
    ...
    if 'image' in source:
        ...
        # image does not exist in the data, but the model is multimodal, insert a empty image. 
        # Let us check if the other SFT case share the same 
...
def make_dpo_data_module(tokenizer, data_args):
    train_dataset = DPODataset(tokenizer=tokenizer,
                                multimodal_cfg=dict(
                                    is_multimodal=data_args.is_multimodal,
                                    # image token length is not needed
...
def train():
    ...
    if training_args.task == 'LM':
        ...
        data_module = make_dpo_data_module(tokenizer=tokenizer,
                                            data_args=data_args)
        trainer = LLavaDPOTrainer(model=model,
                        tokenizer=tokenizer,
                        args=training_args,
                        **data_module)
